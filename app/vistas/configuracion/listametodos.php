...
<table class="table">
  ...
  <tbody>
    <?php
      $num = 1;
      foreach ($datos as $nombre_metodo) :
        if($nombre_metodo != '__construct' && $nombre_metodo != 'modelo' && $nombre_metodo != 'vista' && $nombre_metodo != 'migracionEjecutar'){
    ?>
        <tr>
          <th scope="row"><?= $num; ?></th>
          <td><?= $nombre_metodo ?></td>
          <td></td>
          <td> <a class="btn btn-primary btn-lg btn-block" href="<?= RUTA_URL?>configuraciones/<?= $nombre_metodo ?>">ir</a></td>
        </tr>
