	...
	class TCPDF {
		...
		protected function setHeader() {
			if ($this->print_header) {
				...
				$this->SetFont($this->header_font[0] ?? PDF_FONT_NAME_DATA, $this->header_font[1] ?? '', $this->header_font[2] ?? PDF_FONT_SIZE_DATA);
				$this->Header();
				//restore position
				if ($this->rtl) {
		...
		protected function setFooter() {
			...
			if ($this->print_footer) {
				...
				$this->SetFont($this->footer_font[0] ?? PDF_FONT_NAME_DATA, $this->footer_font[1] ?? '', $this->footer_font[2] ?? PDF_FONT_SIZE_DATA);
				$this->Footer();
				//restore position
				if ($this->rtl) {
					$this->SetXY($this->original_rMargin, $this->tMargin);
		...
		public function unichr($c) {
			// PHP8: chr()/comparaciones numericas ya no aceptan un string vacio o no
			// numerico como antes (TypeError en vez de coercion silenciosa a 0) --
			// se preserva aqui el comportamiento original de PHP7 (chr('') == chr(0)).
			if ($c === '' || $c === null) {
				$c = 0;
			}
			if (!$this->isunicode) {
